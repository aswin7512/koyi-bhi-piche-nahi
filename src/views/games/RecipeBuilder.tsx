...
export const RecipeBuilder = ({ onEnd }: { onEnd: (score: number, time: number) => void }) => {
  ...
  // CHANGED: Track selected ingredient instead of dragged
  ...
  const handleIngredientClick = (itemName: string) => {
    setSelectedIngredient(itemName);
  };
  ...
  const handleBowlClick = () => {
    ...
    if (selectedIngredient === requiredIngredient) {
      ...
      const ingredientIcon = pantry.find(p => p.name === selectedIngredient)?.icon;
  ...
  return (
    <div className="flex flex-col w-full max-w-4xl gap-6 animate-fade-in">
      
      {/* Top Section: Recipe Card & Bowl */}
      <div className="flex flex-col md:flex-row gap-8 items-start justify-center">
        
        {/* Recipe Card */}
        <div className="bg-yellow-50 p-6 rounded-xl border border-yellow-200 shadow-sm rotate-1 transform w-64 mx-auto md:mx-0">
        ...
        <div className="flex-1 flex flex-col items-center justify-center">
          <div
            ...
            className={`relative w-72 h-64 bg-gray-100 rounded-b-[4rem] border-4 border-t-0 transition-all duration-300 overflow-hidden flex items-end justify-center pb-4 cursor-pointer
                 ${selectedIngredient ? 'border-indigo-400 shadow-lg scale-105' : 'border-gray-300 shadow-inner hover:bg-gray-50'}
            `}
          >
            ...
            {/* Visual Guide */}
            {selectedIngredient && (
             <div className="absolute inset-0 flex items-center justify-center bg-white/30 z-20 animate-pulse pointer-events-none rounded-b-[4rem]">
                <span className="text-indigo-600 font-bold flex items-center gap-2 bg-white px-3 py-1 rounded-full shadow-sm">
                  <MousePointerClick size={16} /> Tap to Add
                </span>
             </div>
           )}

            {/* Ingredients in bowl */}
            <div className="flex flex-wrap gap-2 items-end justify-center mb-4 px-4 z-10">
                 {addedIngredients.map((icon, idx) => (
                   <div key={idx} className="animate-bounce-in">{icon}</div>
                 ))}
            </div>
             {/* "Water" level effect */}
            {currentStep > 0 && <div className="absolute bottom-0 left-0 right-0 bg-blue-100/50 h-1/3 w-full -z-0 rounded-b-[3.5rem]"></div>}
          </div>
          {mistakes > 0 && <p className="text-red-500 text-sm mt-2 font-medium">Incorrect ingredients: {mistakes}</p>}
        </div>
      </div>

      {/* Bottom Section: Pantry */}
      <div className="bg-white p-6 rounded-xl border-t border-gray-100 shadow-up-sm">
        <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-4 text-center md:text-left">Pantry Ingredients</h3>
        <div className="flex flex-wrap gap-4 justify-center">
          {pantry.map((item) => (
            <button
              key={item.name}
              onClick={() => handleIngredientClick(item.name)}
              className={`p-4 rounded-xl shadow-sm border-2 flex flex-col items-center transition-all w-24
                ${selectedIngredient === item.name 
                  ? 'bg-indigo-50 border-indigo-500 scale-105 ring-2 ring-indigo-200' 
                  : 'bg-gray-50 border-transparent hover:border-indigo-300 hover:-translate-y-1'
                }
              `}
            >
              {item.icon}
